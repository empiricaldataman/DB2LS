...
-- Check if the table exists or has enabled foreignkeys
   IF @table IS NOT NULL
      ...
      SET @sql_string = 'IF NOT EXISTS (
			...
			IF NOT EXISTS (
				...
				AND f.is_disabled = 0)
			BEGIN
				PRINT ''[' + @database + '].[' + @schema + '].[' + @table + '] does not have any enabled foreign keys.''
			END'
...
WHILE @@FETCH_STATUS = 0
   BEGIN
   SELECT @sql_string = 'ALTER TABLE [' + @database + '].[' + @schema + '].[' + @table + '] NOCHECK CONSTRAINT [' + @foreignkey + ']'
   PRINT @sql_string
   EXEC sp_executesql @sql_string
   FETCH NEXT FROM ForeignKeyCursor 
    INTO @schema, @table, @foreignkey
