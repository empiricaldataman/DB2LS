...
SELECT @sql_string = 'INSERT #table_list SELECT DISTINCT o.object_id, o.type, s.name, o.name FROM [' + @database_name + '].sys.objects o WITH (NOLOCK) JOIN [' + @database_name + '].sys.schemas s WITH (NOLOCK) ON o.schema_id = s.schema_id JOIN [' + @database_name + '].sys.indexes i WITH (NOLOCK) ON o.object_id = i.object_id WHERE o.type IN (''U'',''V'')'
...
DECLARE table_cursor CURSOR 
   ...
   OPEN table_cursor
  FETCH NEXT FROM table_cursor INTO @object_id, @type, @schema_name, @table_name

  WHILE @@FETCH_STATUS = 0
        BEGIN
        SELECT @sql_string = 'SELECT @d = SUM(CASE WHEN (index_id < 2) THEN (in_row_data_page_count + lob_used_page_count + row_overflow_used_page_count)
                     ELSE lob_used_page_count + row_overflow_used_page_count END)
     , @i = SUM(used_page_count) - SUM(CASE WHEN (index_id < 2) THEN (in_row_data_page_count + lob_used_page_count + row_overflow_used_page_count)
                                            ELSE lob_used_page_count + row_overflow_used_page_count END)
