...
-- Check if the table exists or has disabled triggers
    IF @table IS NOT NULL
       ...
       SET @sql_string = '
			...
			BEGIN
				PRINT ''[' + @database + '].[' + @schema + '].[' + @table + '] does not have any disabled triggers.''
			END
			'
...
WHILE @@FETCH_STATUS = 0
      BEGIN
      SELECT @sql_string = 'ENABLE TRIGGER [' + @schema + '].[' + @trigger + '] ON [' + @database + '].[' + @schema + '].[' + @table + ']'
      PRINT @sql_string
      EXEC sp_executesql @sql_string
      FETCH NEXT FROM TriggerCursor 
       INTO @schema, @table, @trigger
