/*-----------------------------------------------------------------------------------------------
        NAME: DBA_SchemaChange_Capture_CJ.sql
...
SELECT @jobId = job_id FROM msdb.dbo.sysjobs WHERE ([name] = N'RDX - Schema Change')

IF (@jobId is NULL)
   BEGIN
   EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'RDX - Schema Change', 
        @enabled=1, 
        ...
        @notify_level_page=0, 
        @delete_level=0, 
        @description=N'Collects sever level schema changes', 
...
IF @enable = 1 
   BEGIN 
        ...
        SELECT @diff AS difference
             ...
             , A.ddl_operation
          FROM #temp_trace A
		  LEFT JOIN [dbo].[SchemaChange] B ON A.[start_time] = B.[start_time]
