...
BEGIN TRANSACTION
      ...
      SELECT @jobId = job_id 
       ...
       WHERE ([name] = N'RDX - XE_Monitor_Performance')

      IF (@jobId is NULL)
         BEGIN
         EXEC @ReturnCode = msdb.dbo.sp_add_job 
              @job_name = N'RDX - XE_Monitor_Performance'
            , @enabled=1
            ...
            , @notify_level_page=0
            , @delete_level=0
            , @description=N'Captures data from extended event XE_Monitor_Performance and saves it in the [dbo].[XE_Monitor_Performance] table.'
...
SELECT i.event_data.value(''(@name)'',''varchar(128)'') [name]
 ...
 INSERT INTO dbo.XE_Monitor_Performance
 SELECT A.* 
   FROM #XE_Monitor_Performance A 
   LEFT JOIN [dbo].[XE_Monitor_Performance] B ON A.[timestamp] = B.timestamp
...
IF EXISTS(SELECT * FROM sys.server_event_sessions WHERE [name]='XE_Monitor_Performance')
   ALTER EVENT SESSION XE_Monitor_Performance  
   ON SERVER  
   STATE = start;  
GO
