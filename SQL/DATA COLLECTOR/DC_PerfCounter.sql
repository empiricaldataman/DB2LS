...
SELECT [collection_time]
     ...
     , ([lazy_writes_sec] - LAG([lazy_writes_sec]) OVER (ORDER BY [collection_time])) / 60. [lazy_writes_sec]
     , [page_life_expectancy] - LAG([page_life_expectancy]) OVER (ORDER BY [collection_time]) [page_life_expectancy]
     , ([full_scans_sec] - LAG([full_scans_sec]) OVER (ORDER BY [collection_time])) / 60. [full_scans_sec]
     , ([index_searches_sec] - LAG([index_searches_sec]) OVER (ORDER BY [collection_time])) / 60. [index_searches_sec]
     , ([batch_requests_sec] - LAG([batch_requests_sec]) OVER (ORDER BY [collection_time])) / 60. [batch_requests_sec]
     , ([sql_compilations_sec] - LAG([sql_compilations_sec]) OVER (ORDER BY [collection_time])) / 60. [sql_compilations_sec]
